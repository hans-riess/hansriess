...
class ProfessionalDevelopment(models.Model):
    """Model for workshops, trainings, certifications, and similar development activities"""

    title = models.CharField(max_length=300, help_text="Title of the activity (e.g., workshop, training)")
    organization = models.CharField(max_length=200, help_text="Hosting organization or provider")
    location = models.CharField(max_length=200, blank=True, null=True, help_text="City, State/Province, Country")
    start_date = models.DateField(blank=True, null=True, help_text="Start date (optional)")
    end_date = models.DateField(blank=True, null=True, help_text="End date (optional)")
    description = models.TextField(blank=True, help_text="Brief description or focus area")
    url = models.URLField(blank=True, null=True, help_text="Link to program or certificate (optional)")
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class Mentorship(models.Model):
    """Model for advising and mentorship relationships"""

    MENTEE_LEVEL_CHOICES = [
    ...
    mentee_name = models.CharField(max_length=200, help_text="Full name of the mentee")
    mentee_level = models.CharField(max_length=20, choices=MENTEE_LEVEL_CHOICES, default='other')
    institution = models.CharField(max_length=200, help_text="Institution or organization of the mentee")
    department = models.CharField(max_length=200, blank=True, null=True, help_text="Department or unit (optional)")
    area = models.CharField(max_length=200, blank=True, null=True, help_text="Area/topic of mentorship (optional)")
    ...
    outcome = models.CharField(max_length=300, blank=True, help_text="Outcome or placement (optional)")
    ...
    def __str__(self):
        end_str = f" - {self.end_date.strftime('%Y')}" if self.end_date else " - Present" if self.is_current else ""
        return f"{self.mentee_name} ({self.get_mentee_level_display()}) {self.institution} ({self.start_date.strftime('%Y')}{end_str})"