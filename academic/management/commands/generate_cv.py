...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        shutil.copy2(os.path.join(settings.BASE_DIR, 'academic', 'tex', 'academic-cv.sty'), os.path.join(temp_dir, 'academic-cv.sty'))
        ...
        for i in range(2): # Run twice for cross-referencing
            ...
            if process.returncode != 0:
                self.stderr.write(self.style.ERROR('PDFLaTeX compilation failed! See log for details.'))
                log_file = os.path.join(temp_dir, 'cv.log')
                try:
                    with open(log_file, 'r') as log:
                        self.stderr.write(log.read())
                except FileNotFoundError:
                    self.stderr.write("Could not find log file. Raw pdflatex output:")
        ...
        if settings.PRODUCTION:
            ...
            if bucket_name:
                self.stdout.write("Production mode: Attempting to upload to S3...")
                try:
                    s3 = boto3.client('s3',
                ...
                self.stdout.write(self.style.WARNING('AWS_STORAGE_BUCKET_NAME not set. Skipping S3 upload.'))
        else:
            # --- Save to local media storage for development ---
            self.stdout.write("Development mode: Saving CV to local media storage.")

            # 1. Delete the old CV file before saving the new one
            if profile.cv:
                self.stdout.write(f"Deleting old CV file: {profile.cv.path}")
                profile.cv.delete(save=False) # Use save=False to avoid an extra database hit

            # 2. Save the new CV
            with open(pdf_file_path, 'rb') as pdf_file:
                # Now, when you save, it will be named 'cv.pdf' without a random suffix
                profile.cv.save('cv.pdf', File(pdf_file), save=True)
                
            self.stdout.write(self.style.SUCCESS(f'Successfully saved new CV to {profile.cv.path}'))
